...
class DriverViewModel extends ChangeNotifier{
  ...
  Future<bool> addDriver(Driver driver, ImageUpload image) async {
    try {
      ...
      var request = http.MultipartRequest(
        'POST',
        Uri.parse(
            '${BaseURL.baseURL}/account/addDriver/${StaticAccount.staticAccount.userNumber}'),
      );
  ...
  Future<bool> deleteDriver(int idDriver)async{

    final String apiUrl = BaseURL.baseURL + '/account/deleteDriver/${idDriver}';
    ...
    try{
      ...
      if (response.statusCode == 200) {
        print('DELETE /deleteDriver  response.status:${response.statusCode}');
      ...
      print("Error delete DRIVER $error");
      throw Exception('Failed delete driver');
